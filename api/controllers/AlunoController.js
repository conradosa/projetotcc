...
module.exports = {
  ...
  etapa2: async function (req, res) {
    try {
      const aluno = await Aluno.findOne({usuario: req.session.usuarioId});
      const orientador = await Professor.findOne({id: aluno.orientador});
      const userOrientador = await Usuario.findOne({id: orientador.usuario});
      return res.view('pages/aluno/etapas/etapa2', {
        professor: userOrientador
      });
    } catch (err) {
      req.session.erro = err.name;
      res.redirect('/aluno');
        ...
        if (err) {
          ...
          try {
            ...
            await Proposta.create({
              diretorio: path.join('/alunos/', matriculastring, 'proposta.pdf'),
              aluno: aluno.id
            }).fetch();
    ...
    req.file('documento').upload({
        ...
        if (err) {
          ...
          try {
            ...
            await Previa.create({
              diretorio: path.join('/alunos/', matriculastring, 'previa.pdf'),
        ...
        if (err) {
          ...
          try {
            ...
            await Documentacao.create({
              diretorio: path.join('/alunos/', matriculastring, 'documentacao.pdf'),
              aluno: aluno.id
            }).fetch();
